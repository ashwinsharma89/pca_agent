...
class TestBenchmarkRetrieval:
    ...
    def test_b2c_benchmark_retrieval(self, benchmark_engine):
        ...
        b2b_context = {'business_model': 'B2B', 'industry': 'Technology'}
        b2b_benchmarks = benchmark_engine.get_contextual_benchmarks(b2b_context)
        
        # At least some benchmarks should be different
        assert (benchmarks['ctr'] != b2b_benchmarks['ctr'] or 
                benchmarks['roas'] != b2b_benchmarks['roas'])
    ...
    def test_channel_specific_benchmarks(self, benchmark_engine):
        ...
        linkedin_benchmarks = benchmark_engine.get_contextual_benchmarks(linkedin_context)
        
        assert 'ctr' in linkedin_benchmarks
        ...
        meta_benchmarks = benchmark_engine.get_contextual_benchmarks(meta_context)
        
        assert 'ctr' in meta_benchmarks
        
        # Benchmarks should differ by channel
        assert linkedin_benchmarks != meta_benchmarks
    ...
    def test_industry_specific_benchmarks(self, benchmark_engine):
        ...
        tech_benchmarks = benchmark_engine.get_contextual_benchmarks(tech_context)
        ...
        healthcare_benchmarks = benchmark_engine.get_contextual_benchmarks(healthcare_context)
        
        # Both should return valid benchmarks
        assert 'ctr' in tech_benchmarks
        assert 'ctr' in healthcare_benchmarks
    ...
    def test_benchmark_fallback(self, benchmark_engine):
        ...
        # Request benchmarks for uncommon scenario
        context = {
            'business_model': 'B2B',
            'industry': 'Rare Industry',
            'channel': 'Uncommon Channel'
        }
        
        benchmarks = benchmark_engine.get_contextual_benchmarks(context)
        
        # Should still return default benchmarks
        assert 'ctr' in benchmarks
        assert 'roas' in benchmarks
        assert benchmarks['ctr'] > 0
...
class TestRetrievalPerformance:
    ...
    def test_batch_retrieval(self):
        ...
        contexts = [
            {'business_model': 'B2B', 'channel': 'LinkedIn'},
            {'business_model': 'B2C', 'channel': 'Meta'},
            {'business_model': 'B2B', 'channel': 'Google Ads'}
        ]
        ...
        assert all('ctr' in r for r in results)
    ...
    def test_caching_effectiveness(self):
        ...
        # First call (cold)
        start_time = time.time()
        result1 = benchmark_engine.get_contextual_benchmarks(context)
        first_call_time = time.time() - start_time
        
        # Second call (should be cached)
        start_time = time.time()
        result2 = benchmark_engine.get_contextual_benchmarks(context)

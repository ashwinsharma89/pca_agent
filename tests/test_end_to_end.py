...
class TestEndToEndWorkflow:
    ...
    def test_full_campaign_analysis(self, sample_campaign_data, campaign_context):
        ...
        specialist = router.route_to_specialist(google_data.iloc[0].to_dict())
        ...
        pattern_analysis = reasoning_agent.analyze_with_patterns(
        ...
        assert len(pattern_analysis['insights']) > 0
    ...
    def test_workflow_with_filters(self, sample_campaign_data, campaign_context):
        ...
        reasoning_agent = EnhancedReasoningAgent()
        analysis = reasoning_agent.analyze_with_patterns(
    ...
    def test_multi_channel_analysis(self, sample_campaign_data, campaign_context):
        ...
        for channel in channels:
            ...
            # Route to specialist
            specialist = router.route_to_specialist(channel_data.iloc[0].to_dict())
            assert specialist is not None
            
            # Get channel-specific insights
            insights = specialist.analyze(channel_data.to_dict('records'))
    ...
    def test_benchmark_comparison(self, sample_campaign_data, campaign_context):
        ...
        # Verify benchmark adjustment based on context
        b2c_context = campaign_context.copy()
        b2c_context['business_model'] = 'B2C'
        
        b2c_benchmarks = benchmark_engine.get_contextual_benchmarks(b2c_context)
        
        # B2C and B2B benchmarks should differ
        assert b2c_benchmarks['ctr'] != benchmarks['ctr']
    ...
    def test_error_handling(self, campaign_context):
        ...
        try:
            ...
            # Should not crash
            assert False, f"Failed to handle empty data: {e}"
        ...
        try:
            filter_engine = SmartFilterEngine()
            suggestions = filter_engine.suggest_filters(
                incomplete_data,
                campaign_context
            )
            # Should handle gracefully
            ...
            assert False, f"Failed to handle incomplete data: {e}"
